...
def simple_python_constructor():
    global tas
    # Try common near-surface air temperature variable names
    try:
        tas = comin.var_get(
            [comin.EP_ATM_WRITE_OUTPUT_BEFORE], ("tas", jg), flag=comin.COMIN_FLAG_READ
        )
        print("Using near-surface air temperature variable: tas", file=sys.stderr)
    except:
        try:
            tas = comin.var_get(
                [comin.EP_ATM_WRITE_OUTPUT_BEFORE], ("temp2", jg), flag=comin.COMIN_FLAG_READ
            )
            print("Using near-surface air temperature variable: temp2", file=sys.stderr)
        except:
            try:
                tas = comin.var_get(
                    [comin.EP_ATM_WRITE_OUTPUT_BEFORE], ("t_2m", jg), flag=comin.COMIN_FLAG_READ
                )
                print("Using near-surface air temperature variable: t_2m", file=sys.stderr)
            except Exception as e:
                print(f"ERROR: Could not find near-surface air temperature variable. Tried: tas, temp2, t_2m", file=sys.stderr)
...
def build_neighbor_map(n_coarse=80):
    ...
    ⚠️  IMPORTANT LIMITATION - PLACEHOLDER IMPLEMENTATION:
    This function does NOT use actual ICON grid topology. It creates artificial
    cyclic neighbors that don't correspond to real spatial adjacency.
    
    For production use, this should be replaced with:
        neighbor_map = domain.cells.neighbor_idx[::256]  # Sample every 256th cell
    to get the actual hexagonal neighbor relationships from ICON grid metadata.
    
    Args:
        n_coarse: Number of coarse cells (80 for R2B01)
    Returns:
        neighbor_map: (n_coarse, 6) array of neighbor indices [ARTIFICIAL, NOT REAL TOPOLOGY]
    """
    # Temporary placeholder: cyclic neighbors (NOT spatially meaningful)
    # TODO: Replace with actual ICON grid topology from domain.cells.neighbor_idx
...
def get_batch_callback():
    global net, optimizer, neighbor_map, training_counter, device
    ...
    if 'device' not in globals():
        ...
        if torch.cuda.is_available():
            print(f"🚀 GPU ACCELERATION ENABLED: {torch.cuda.get_device_name(0)}", file=sys.stderr)
            print(f"   GPU Memory: {torch.cuda.get_device_properties(0).total_memory / 1e9:.1f} GB", file=sys.stderr)
        else:
            print(f"⚠️  Running on CPU (GPU not available)", file=sys.stderr)
    
    # ============================================
    # TRAINING CUTOFF CONFIGURATION
    # ============================================
    # Set the date after which training stops and test data is saved
    # Example: Train on first 7 days, save last 3 days for testing
    TRAINING_END_DATE = "1979-01-05T12:00:00"  # Stop training after Jan 5, 1979 12:00 UTC
    
    # Gather fine-resolution near-surface air temperature (R2B04: 20480 cells)
    ...
    # Check if we're past the training cutoff date
    training_cutoff_np = pd.to_datetime(TRAINING_END_DATE)
    is_test_phase = current_time_np >= training_cutoff_np
    ...
    if rank == 0:
        ...
        # Validate dimensions
        if tas_np_glb.shape[0] != N_R2B04:
            print(f"ERROR: Expected {N_R2B04} cells but got {tas_np_glb.shape[0]}", file=sys.stderr)
            return
        
        # Validate input data for NaN/Inf
        if np.any(~np.isfinite(tas_np_glb)):
            print("ERROR: Near-surface air temperature contains NaN or Inf values!", file=sys.stderr)
            return
        
        # ============================================
        # Coarsen R2B04 → R2B01 (average 256 cells → 1)
        # ============================================
        # ⚠️  CRITICAL CLARIFICATION: HOW ICON STORES GRID DATA
        # 
        # ICON Grid Cell Ordering (Space-Filling Curve):
        # -----------------------------------------------
        # ICON uses a hierarchical refinement scheme where:
        #   - R2B01 has 80 cells (base icosahedron with 1 bisection)
        #   - R2B04 has 20480 cells (4 bisections = 2^8 = 256× refinement)
        # 
        # Storage rule during ICON run:
        #   The util_gather() function (lines 75-100) collects data from all MPI ranks
        #   and reorders cells by their GLOBAL INDEX (domain.cells.glb_index).
        #   Result: tas_np_glb[i] is the value for the i-th cell in ICON's canonical ordering.
        # 
        # ICON's canonical ordering follows a space-filling curve (Hilbert-like):
        #   - Parent cell k at level n has child cells at indices that are NOT necessarily
        #     k*4 to k*4+3 in the refined grid
        #   - The exact parent-child mapping requires the grid file metadata
        # 
        # What this code does (APPROXIMATE):
        # ----------------------------------
        # Simple block averaging: assumes cells [0:256] → coarse cell 0, [256:512] → cell 1, etc.
        # This is WRONG because ICON's space-filling curve doesn't group children consecutively.
        # 
        # Why it might still "work":
        #   - The space-filling curve has locality: nearby indices → nearby spatial locations
        #   - Averaging 256 consecutive cells gives "roughly" the right coarse value
        #   - But it's NOT the exact parent cell value, just an approximation
        # 
        # For CORRECT coarsening, you need:
        #   1. Load grid file: icon_grid_R2B04_G.nc and icon_grid_R2B01_G.nc
        #   2. Use parent_cell_index[] array to map fine → coarse
        #   3. For each R2B01 cell i, average all R2B04 cells where parent_cell_index == i
        # 
        # Current approach: Approximate coarsening by block averaging (NOT exact ICON topology)
        tas_r2b04_fine = tas_np_glb.reshape(N_R2B01, N_FINE_PER_COARSE)  # Shape: (80, 256)
        tas_r2b01_coarse = tas_r2b04_fine.mean(axis=1)  # Shape: (80,) - average each block of 256 cells
        ...
        tas_coarse_context = np.zeros((N_R2B01, N_NEIGHBORS))
        tas_coarse_context[:, 0] = tas_r2b01_coarse  # Center cell
        for i in range(N_R2B01):
            tas_coarse_context[i, 1:] = tas_r2b01_coarse[neighbor_map[i]]
        
        # ============================================
        # Data Normalization with EMA
        # ============================================
        # Current batch statistics (use center values only for statistics)
        current_coarse_mean = tas_r2b01_coarse.mean()
        current_coarse_std = tas_r2b01_coarse.std()
        current_fine_mean = tas_np_glb.mean()
        current_fine_std = tas_np_glb.std()
        
        print(f"Conditional diffusion statistics at {current_time_np}:", file=sys.stderr)
        print(f"  Current - Coarse: mean={current_coarse_mean:.6e}, std={current_coarse_std:.6e}, range=[{tas_r2b01_coarse.min():.6e}, {tas_r2b01_coarse.max():.6e}]", file=sys.stderr)
        print(f"  Current - Fine:   mean={current_fine_mean:.6e}, std={current_fine_std:.6e}, range=[{tas_np_glb.min():.6e}, {tas_np_glb.max():.6e}]", file=sys.stderr)
        ...
        # ============================================
        # STORAGE LOCATIONS (answers to "where is everything stored?"):
        # ============================================
        # 1. MODEL WEIGHTS & OPTIMIZER STATE:
        #    Saved to disk: /scratch/{user}/icon_exercise_comin/net_{start_time}.pth
        #    This PyTorch checkpoint contains:
        #      - model_state_dict: Neural network weights
        #      - optimizer_state_dict: Adam optimizer state (momentum, learning rate)
        #      - EMA statistics: Running mean/std for normalization
        #      - replay_buffer: In-memory training data (see below)
        #      - training_counter: Number of training sessions completed
        #
        # 2. TRAINING LOSSES:
        #    Saved to disk: /scratch/{user}/icon_exercise_comin/log_session_{counter}_{time}.txt
        #    Each training session creates a new log file with epoch losses
        #
        # 3. REPLAY BUFFER (Training Dataset):
        #    - During ICON runtime: Stored IN MEMORY as Python list 'replay_buffer'
        #      Size: 48 timesteps × 80 samples × (7 context + 256 fine values)
        #            ≈ 48 × 80 × 263 × 8 bytes ≈ 8 MB (negligible memory footprint)
        #    - Between ICON runs: Saved to DISK in the checkpoint file
        #      This allows training to resume with the same historical data
        #
        # 4. LOSS FUNCTION:
        #    Defined in code: torch.nn.MSELoss() (Mean Squared Error)
        #    Not stored separately - recreated each time
        # ============================================
        ...
        if is_first_run:
            ...
            # This is an IN-MEMORY Python list that accumulates training samples
            # Format: [(context_80x7, fine_80x256), (context_80x7, fine_80x256), ...]
            # During runtime: kept in RAM (global variable)
            # Between runs: persisted to disk in checkpoint file
            replay_buffer = []
        ...
        # ============================================
        tas_coarse_context_norm = (tas_coarse_context - ema_coarse_mean) / ema_coarse_std
        tas_fine_norm = (tas_r2b04_fine - ema_fine_mean) / ema_fine_std
        
        # ============================================
        # TEST PHASE: Save data for testing (no training)
        # ============================================
        if is_test_phase:
            import os
            test_data_dir = f"/scratch/{user[0]}/{user}/icon_exercise_comin/test_data_{start_time_str}"
            os.makedirs(test_data_dir, exist_ok=True)
            
            # Save test data: coarse context + fine field (normalized and unnormalized)
            test_data_path = os.path.join(test_data_dir, f"test_sample_{current_time_str}.npz")
            np.savez(
                test_data_path,
                # Normalized data (ready for model input)
                coarse_context_norm=tas_coarse_context_norm,
                fine_field_norm=tas_fine_norm,
                # Unnormalized data (for visualization)
                coarse_context=tas_coarse_context,
                fine_field=tas_r2b04_fine,
                ...
                simulation_time=str(current_time_np)
            )
            print(f"TEST PHASE: Saved test data at {current_time_np} to {test_data_path}", file=sys.stderr)
            print(f"  Coarse context shape: {tas_coarse_context.shape}, Fine field shape: {tas_r2b04_fine.shape}", file=sys.stderr)
            return  # Skip all training logic
        
        # ============================================
        # TRAINING PHASE: Continue normal training logic
        # ============================================
        print(f"TRAINING PHASE: Processing timestep at {current_time_np}", file=sys.stderr)
        
        # ============================================
        # Experience Replay Buffer (Only during training)
        # ============================================
        # Add current timestep to buffer (context + fine field)
        # This stays IN MEMORY during ICON simulation (global variable 'replay_buffer')
        # Rolling window: oldest data is removed when buffer exceeds BUFFER_SIZE
        current_samples = (tas_coarse_context_norm.copy(), tas_fine_norm.copy())
        replay_buffer.append(current_samples)
        
        # Keep only last BUFFER_SIZE timesteps (FIFO queue)
        if len(replay_buffer) > BUFFER_SIZE:
            replay_buffer.pop(0)  # Remove oldest sample
        ...
        for epoch in range(N_EPOCHS):
            ...
            indices = torch.randperm(n_total_samples, device=device)
            epoch_loss = 0.0
            n_batches = 0
            ...
            for i in range(0, n_total_samples, BATCH_SIZE):
                ...
                t = torch.randint(0, N_DIFFUSION_STEPS, (context_batch.shape[0],), dtype=torch.long, device=device)
