...
def build_knn_graph_batch_numpy(pos, batch_nodes, k=6, extended_k=8):
    ...
    for node_id in batch_nodes:
        ...
        nearest = np.argpartition(dists, min(extended_k+1, num_all_nodes))[:extended_k+1]
    ...
    for local_i, global_i in enumerate(batch_node_ids):
        ...
        nearest_local = np.argpartition(dists, min(k+1, num_batch_nodes))[:min(k+1, num_batch_nodes)]
    ...
    batch_edge_index = np.array(edges_list).T if edges_list else np.zeros((2, 0), dtype=np.int64)
...
class SimpleGNN(nn.Module):
    ...
    def message_passing(self, x, edge_index, message_mlp):
        ...
        messages = torch.cat([src_features, dst_features], dim=1)  # [num_edges, 2*features]
...
def get_batch_callback():
    ...
    start_time_str = str(start_time).replace(" ", "_").replace(":", "-")  # Safe filename
    ...
    if rank == 0:
        ...
        if is_first_timestep:
            ...
            print(f"✓ Model loaded from checkpoint at {current_time_str}", file=sys.stderr)
        ...
        if use_gnn:
            ...
            for batch_idx in range(num_batches):
                ...
                print(f"\n  📦 Batch {batch_idx+1}/{num_batches}: nodes [{start_node}:{end_node}]", file=sys.stderr)
                ...
                print(f"     Subgraph: {len(batch_node_ids)} nodes, {num_batch_edges} edges", file=sys.stderr)
            ...
            for i in range(num_batches):
                ...
                losses.append(loss.item())
                if i % 50 == 0:
                    print(f"  Batch {i+1}/{num_batches}: Loss = {loss.item():.6e}", file=sys.stderr)
